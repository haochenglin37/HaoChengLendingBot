...
class Poloniex(ExchangeApi):
    def __init__(self, cfg, log):
        ...
        secret = self.cfg.get("API", "secret", None)
    ...
    def api_query(self, command, req=None):
        ...
        try:
            if command == "returnTicker" or command == "return24hVolume":
                ...
                ret = urllib.request.urlopen(urllib.request.Request(req_url))
                return _read_response(ret)
            else:
                req['command'] = command
            ...
            try:
                data = json.loads(raw_polo_response.decode('utf-8'))
                ...
                if hasattr(ex, 'code') and (ex.code == 502 or ex.code in range(520, 527, 1)):
                    ...
                    polo_error_msg = raw_polo_response.decode('utf-8', errors='ignore')
            message = "{0} Requesting {1}.  Poloniex reports: '{2}'".format(str(ex), command, polo_error_msg)
            raise Exception(message)
        except Exception as ex:
            message = "{0} Requesting {1}".format(str(ex), command)
            raise Exception(message)
