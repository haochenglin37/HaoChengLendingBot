...
class Bitfinex(ExchangeApi):
    ...
    def _request(self, method, request, payload=None, verify=True):
        try:
            ...
            message = "{0} Requesting {1}".format(str(ex), self.url + request)
    ...
    def create_loan_offer(self, currency, amount, duration, auto_renew, lending_rate):
        ...
        try:
            ...
            if "Invalid offer: incorrect amount, minimum is 50" in msg:
                ...
                if currency != 'USD':
                    if currency == 'EUR':
                        cur_min = usd_min / float(self.return_ticker()['USD_BTC']['lowestAsk']) * \
                                  float(self.return_ticker()['EUR_BTC']['lowestAsk'])
                    else:
    ...
    def return_balances(self):
        ...
        return_dict = {cur: '0.00000000' for cur in self.cfg.get_all_currencies()}
    ...
    def get_public_funding_trades(self, symbol, start=None, end=None, limit=250, sort=1):
        """
        Retrieves public funding trades for the supplied symbol (e.g. fUSD).
        ...
        command = 'trades/{0}/hist?{1}'.format(symbol, query)
        return self._get(command, 'v2')
