...
auto_cancel_open_offers = Config.getboolean('BOT', 'autoCancelOpenOffers', True)

# Cancel any legacy open offers once at startup so only new strategy orders remain
try:
    Lending.cancel_all()
except Exception as ex:
    log.log("Initial cancel_all failed: {0}".format(ex))
    ...
    while True:
        try:
            ...
            if 'Invalid API key' in message:
                ...
                exit(1)
            elif 'Nonce must be greater' in message:
                print("!!! Troubleshooting !!!")
            ...
            # Ignore all 5xx errors (server error) as we can't do anything about it (https://httpstatuses.com/)
                ...
                print(("Caught {0} from exchange, ignoring.".format(message)))
            elif isinstance(ex, ApiError):
                print(("Caught {0} reading from exchange API, ignoring.".format(message)))
            else:
